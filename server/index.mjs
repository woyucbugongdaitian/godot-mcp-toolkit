...
const coreTools = [
  ...
  tool("list_export_presets", "Read named Godot export presets from export_presets.cfg.", projectSchema(), async ({ projectPath }) => listExportPresets(requireProject(projectPath))),
  tool("export_project", "Export a Godot project with a named preset to a safe project-relative output path.", { type: "object", required: ["projectPath", "preset", "outputPath"], properties: { projectPath: { type: "string" }, preset: { type: "string" }, outputPath: { type: "string" }, debug: { type: "boolean", default: false }, godotBinary: { type: "string" }, timeoutMs: { type: "integer", minimum: 1000, maximum: 900000, default: 300000 } }, additionalProperties: false }, async ({ projectPath, preset, outputPath, debug = false, godotBinary, timeoutMs = 300000 }) => exportProject(requireProject(projectPath), preset, outputPath, debug, godotBinary, timeoutMs)),
...
const TOOL_GROUPS = {
  ...
  deep_authoring: ["edit_animation_state_machine", "edit_animation_curve", "configure_animation_timeline", "edit_theme_resource", "edit_editor_resource", "assign_editor_resource", "edit_tileset_atlas", "configure_tileset_terrain", "configure_tileset_proxy", "paint_tilemap_terrain", "edit_array_mesh", "edit_skeleton_3d", "edit_skin_resource"],
  professional_workflows: ["edit_editor_script", "inspect_editor_imports", "bake_navigation_region", "edit_editor_audio_bus_effects"],
  runtime_diagnostics: ["get_runtime_performance_snapshot", "get_runtime_body_contacts", "get_runtime_audio_analysis", "edit_runtime_audio_bus_effects"],
  project: ["create_project", "get_project_info", "set_project_setting", "list_projects", "list_export_presets", "export_project"],
  ...
  runtime: ["launch_editor", "run_project", "get_debug_output", "stop_project", "simulate_input", "run_automation_test", "get_runtime_info", "get_runtime_scene_tree", "get_runtime_node_properties", "set_runtime_property", "call_runtime_method", "pause_runtime", "resume_runtime", "step_runtime", "send_runtime_input", "inject_runtime_pointer", "configure_runtime_observability", "poll_runtime_observability", "capture_runtime_screenshot", "query_runtime_physics", "query_runtime_physics_shape", "query_runtime_navigation_path", "get_runtime_audio_state", "set_runtime_audio_bus", "play_runtime_audio", "get_runtime_logs", "get_runtime_performance_snapshot", "get_runtime_body_contacts", "get_runtime_audio_analysis", "edit_runtime_audio_bus_effects", "get_runtime_binding", "release_runtime_binding"],
...
const liveEditorGroups = new Set(["editor", "ui", "effects", "advanced_editor", "deep_authoring", "professional_workflows"]);
...
function createRuntimeTools() {
  return [
    ...
    runtimeTool("get_runtime_logs", "Read structured logs emitted by the running game's MCP runtime agent with cursor and severity filters.", "get_logs", { tailLines: { type: "integer", minimum: 1, maximum: 1000, default: 200 }, since: { type: "integer", minimum: 0, default: 0 }, severity: { type: "string", enum: ["debug", "info", "warning", "error"] }, source: { type: "string" }, contains: { type: "string", maxLength: 512 } }),
    runtimeTool("get_runtime_performance_snapshot", "Read an immediate runtime performance and scene-composition snapshot.", "get_performance_snapshot", {}),
    runtimeTool("get_runtime_body_contacts", "Inspect contact-monitor data from a running RigidBody2D or RigidBody3D.", "get_body_contacts", { nodePath: { type: "string" } }, ["nodePath"]),
    runtimeTool("get_runtime_audio_analysis", "Read live audio bus peaks and an optional spectrum-analyzer frequency range.", "get_audio_analysis", { bus: { type: "string", default: "Master" }, fromHz: { type: "number", minimum: 1, default: 20 }, toHz: { type: "number", minimum: 1, default: 20000 }, spectrumMode: { type: "integer", minimum: 0, maximum: 1, default: 0 } }),
    runtimeTool("edit_runtime_audio_bus_effects", "Inspect, add, remove, or configure effects on a live AudioServer bus.", "edit_audio_bus_effects", { action: { type: "string", enum: ["inspect", "add", "remove", "configure"] }, bus: { type: "string", default: "Master" }, effectType: { type: "string" }, effectIndex: { type: "integer", minimum: 0 }, index: { type: "integer", minimum: -1, default: -1 }, properties: { type: "object", additionalProperties: true } }, ["action"]),
...
function createAdvancedEditorTools() {
  return [
    ...
    editorTool("edit_editor_script", "Inspect and modify the active Script Editor buffer: caret, selection, text insertion, replacement, and completion requests.", "edit_editor_script", { action: { type: "string", enum: ["inspect", "set_caret", "set_selection", "clear_selection", "insert_text", "replace_selection", "set_text", "request_completion"] }, line: { type: "integer", minimum: 1, default: 1 }, column: { type: "integer", minimum: 0, default: 0 }, fromLine: { type: "integer", minimum: 1 }, fromColumn: { type: "integer", minimum: 0 }, toLine: { type: "integer", minimum: 1 }, toColumn: { type: "integer", minimum: 0 }, text: { type: "string", maxLength: 1048576 } }, ["action"]),
    ...
    editorTool("edit_animation_state_machine", "Create, inspect, and edit AnimationTree state machines, transitions, state placement, and BlendSpace points.", "edit_animation_state_machine", { animationTreePath: { type: "string" }, action: { type: "string", enum: ["create", "inspect", "add_state", "remove_state", "set_state_position", "add_transition", "remove_transition", "configure_transition", "add_blend_space", "add_blend_point", "configure_blend_space"] }, stateName: { type: "string" }, animationPath: { type: "string" }, position: { type: "object", additionalProperties: true }, fromState: { type: "string" }, toState: { type: "string" }, xfadeTime: { type: "number", minimum: 0, default: 0.2 }, reset: { type: "boolean", default: false }, priority: { type: "integer", default: 1 }, properties: { type: "object", additionalProperties: true }, blendSpaceDimension: { type: "string", enum: ["1d", "2d"], default: "1d" }, blendState: { type: "string" }, blendPosition: {} }, ["animationTreePath", "action"]),
    ...
    editorTool("edit_theme_resource", "Inspect and save Theme colors, constants, fonts, icons, StyleBoxes, and type-variation inheritance.", "edit_theme_resource", { resourcePath: { type: "string" }, action: { type: "string", enum: ["inspect", "set_item", "clear_item", "set_type_variation"] }, itemType: { type: "string", enum: ["color", "constant", "font_size", "font", "icon", "stylebox"] }, controlType: { type: "string", default: "Control" }, name: { type: "string" }, value: {}, resourceValuePath: { type: "string" }, style: { type: "object", additionalProperties: true }, baseType: { type: "string" } }, ["resourcePath", "action"]),
    editorTool("edit_editor_resource", "Inspect, duplicate, or save serializable properties of a project Resource.", "edit_editor_resource", { action: { type: "string", enum: ["inspect", "set_properties", "duplicate"] }, resourcePath: { type: "string" }, destinationPath: { type: "string" }, properties: { type: "object", additionalProperties: true }, maxProperties: { type: "integer", minimum: 1, maximum: 512, default: 128 } }, ["action", "resourcePath"]),
    ...
    editorTool("edit_tileset_atlas", "Inspect a TileSet atlas or create sources, tiles, and alternative tiles.", "edit_tileset_atlas", { tileMapPath: { type: "string" }, action: { type: "string", enum: ["inspect", "create_source", "create_tile", "create_alternative"] }, texturePath: { type: "string" }, sourceId: { type: "integer", minimum: 0 }, regionSize: { type: "object", additionalProperties: true }, tileCoordinates: { type: "object", additionalProperties: true }, tileSize: { type: "object", additionalProperties: true }, alternativeId: { type: "integer", minimum: 0 } }, ["tileMapPath", "action"]),
    editorTool("configure_tileset_terrain", "Inspect and define TileSet terrain sets, terrain entries, and peering bits for a TileMap.", "configure_tileset_terrain", { tileMapPath: { type: "string" }, action: { type: "string", enum: ["inspect", "add_set", "add_terrain", "configure_set", "configure_terrain", "set_peering_bit"] }, terrainSet: { type: "integer", minimum: 0 }, terrain: { type: "integer", minimum: 0 }, mode: { type: "integer", minimum: 0 }, name: { type: "string" }, color: { type: "string" }, peeringBit: { type: "integer", minimum: 0 }, peeringTerrain: { type: "integer", minimum: -1 } }, ["tileMapPath", "action"]),
    editorTool("configure_tileset_proxy", "Set or remove source, coordinate, or full TileSet proxy mappings for a TileMap.", "configure_tileset_proxy", { tileMapPath: { type: "string" }, action: { type: "string", enum: ["set_source", "remove_source", "set_coords", "remove_coords", "set_tile", "remove_tile"] }, sourceFrom: { type: "integer", minimum: 0 }, sourceTo: { type: "integer", minimum: 0 }, coordinatesFrom: { type: "object", additionalProperties: true }, coordinatesTo: { type: "object", additionalProperties: true }, alternativeFrom: { type: "integer", minimum: 0, default: 0 }, alternativeTo: { type: "integer", minimum: 0, default: 0 } }, ["tileMapPath", "action", "sourceFrom"]),
    ...
    editorTool("edit_skeleton_3d", "Create, inspect, and edit Skeleton3D bones, parents, and rest transforms.", "edit_skeleton_3d", { action: { type: "string", enum: ["create", "inspect", "add_bone", "set_bone_parent", "set_bone_rest"] }, parentPath: { type: "string", default: "." }, nodePath: { type: "string" }, nodeName: { type: "string", default: "McpSkeleton3D" }, boneName: { type: "string" }, boneIndex: { type: "integer", minimum: 0 }, parentBone: { type: "integer", minimum: -1, default: -1 }, restTransform: { type: "object", additionalProperties: true } }, ["action"]),
    editorTool("edit_skin_resource", "Create, inspect, and extend a saved Skin resource with bone binds.", "edit_skin_resource", { action: { type: "string", enum: ["create", "inspect", "add_bind"] }, resourcePath: { type: "string" }, bone: { type: "integer", minimum: 0 }, bindPose: { type: "object", additionalProperties: true } }, ["action"]),
    ...
    editorTool("inspect_editor_imports", "Inspect import validity and resource types through Godot's live EditorFileSystem.", "inspect_editor_imports", { paths: { type: "array", items: { type: "string" }, minItems: 1, maxItems: 256 } }, ["paths"]),
    editorTool("bake_navigation_region", "Create a NavigationMesh if needed and start an asynchronous bake for a NavigationRegion3D.", "bake_navigation_region", { nodePath: { type: "string" }, onThread: { type: "boolean", default: true } }, ["nodePath"]),    editorTool("inspect_navigation", "Inspect navigation region and agent nodes in the edited scene.", "inspect_navigation", { rootPath: { type: "string", default: "." } }),
    ...
    editorTool("edit_editor_audio_bus_effects", "Inspect, add, remove, or configure AudioServer bus effects through the editor bridge.", "edit_editor_audio_bus_effects", { action: { type: "string", enum: ["inspect", "add", "remove", "configure"] }, bus: { type: "string", default: "Master" }, effectType: { type: "string" }, effectIndex: { type: "integer", minimum: 0 }, index: { type: "integer", minimum: -1, default: -1 }, properties: { type: "object", additionalProperties: true } }, ["action"]),    editorTool("inspect_editor_audio_nodes", "Inspect audio player nodes in the edited scene.", "inspect_editor_audio_nodes", { rootPath: { type: "string", default: "." } }),
...
async function listExportPresets(projectPath) {
  const configPath = join(projectPath, "export_presets.cfg");
  let content;
  try {
    content = await fs.readFile(configPath, "utf8");
  } catch {
    return { projectPath, configPath: "export_presets.cfg", presets: [], exists: false };
  }
  const presets = [];
  let current = null;
  for (const rawLine of content.split(/\r?\n/)) {
    const line = rawLine.trim();
    const section = line.match(/^\[preset\.(\d+)\]$/);
    if (section) {
      current = { index: Number(section[1]), name: "", platform: "", runnable: false };
    ...
    if (key === "name") current.name = unquoted;
    else if (key === "platform") current.platform = unquoted;
    else if (key === "runnable") current.runnable = value === "true";
    else if (key === "export_filter") current.exportFilter = unquoted;
  }
  return { projectPath, configPath: "export_presets.cfg", presets, exists: true };
}

async function exportProject(projectPath, preset, outputPath, debug, explicitBinary, timeoutMs) {
  const presetListing = await listExportPresets(projectPath);
  const selectedPreset = presetListing.presets.find((entry) => entry.name === preset);
  if (!selectedPreset) throw new McpError(-32602, `Export preset not found: ${preset}`);
  const targetPath = resolveInside(projectPath, outputPath);
  await fs.mkdir(dirname(targetPath), { recursive: true });
  const binary = findGodotBinary(explicitBinary);
  if (!binary) throw new McpError(-32001, "Godot executable not found. Set GODOT_BIN or GODOT_PATH, or pass godotBinary.");
  const result = await runProcess(binary, ["--headless", "--path", projectPath, debug ? "--export-debug" : "--export-release", preset, targetPath], { timeoutMs });
  return { projectPath, preset, platform: selectedPreset.platform, debug, outputPath, absoluteOutputPath: targetPath, ok: result.code === 0, exitCode: result.code, signal: result.signal, stdout: result.stdout, stderr: result.stderr, diagnostics: `${result.stdout}\n${result.stderr}`.trim() };
}
