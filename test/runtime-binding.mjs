...
const projectPath = resolve(tmpdir(), `godot-mcp-toolkit-runtime-binding-${process.pid}`);
const otherProjectPath = resolve(tmpdir(), `godot-mcp-toolkit-runtime-binding-other-${process.pid}`);

await fs.mkdir(projectPath, { recursive: true });
await fs.mkdir(otherProjectPath, { recursive: true });
await fs.writeFile(resolve(projectPath, "project.godot"), '[application]\nconfig/name="Runtime Binding Test"\n', "utf8");
...
function bindingStatus() {
  return {
    ...
    projectPath: boundSessionId ? projectPath : null,
  ...
  socket.on("message", (raw) => {
    ...
    if (request.operation === "release_session") {
      ...
      if (boundSessionId !== params.sessionId) {
        send(false, null, "Only the bound MCP conversation can release this runtime", { code: "runtime_binding_conflict", projectPath });
        return;
      }
      boundSessionId = null;
    ...
    if (boundSessionId && boundSessionId !== params.sessionId) {
      send(false, null, "Runtime is already bound to another MCP conversation", { code: "runtime_binding_conflict", projectPath, leaseSecondsRemaining: 90 });
      return;
    }
    boundSessionId = params.sessionId;
    if (request.operation === "get_runtime_info") {
      send(true, { projectPath, runtimePort: bridgePort, paused: false, binding: bindingStatus() });
      return;
    }
...
function createClient(label) {
  const child = spawn(process.execPath, [serverPath], {
    env: {
      ...
      GODOT_MCP_RUNTIME_URL: bridgeUrl,
...
try {
  ...
  const secondBindingConflict = await secondClient.call("get_runtime_info");
  assert.match(secondBindingConflict.error, /already bound to another MCP conversation/);

  const released = await firstClient.call("release_runtime_binding");
  assert.equal(released.value.released, true);
  assert.equal(released.value.binding.bound, false);

  const secondInfo = await secondClient.call("get_runtime_info");
  assert.equal(secondInfo.value.binding.bound, true);

  const firstConflict = await firstClient.call("get_runtime_info");
  assert.match(firstConflict.error, /already bound to another MCP conversation/);

  console.log("Runtime binding checks passed");
} finally {
